...
<div class="row">
  <div class="col-md-4">
    <select class="form-control" onchange="location = this.value;">
      <option value="<?php echo Yii::app()->request->baseUrl?>/piadmin/Customer/Report" >Khách hàng có hợp đồng đang hiệu lực</option>
      <option value="<?php echo Yii::app()->request->baseUrl?>/piadmin/Customer/ReportOther" selected="selected">Khách hàng tiềm năng và HĐ đã tất toán</option>
    </select>
  </div>
</div>
<div class="row">
...
<script type="text/javascript">
  var def = [];
    ...
    var tmp = 0;
		$('.cusID').each(function(index){
      var val = $(this).val();
      setTimeout(callRequestSendMail(val), 5000);
    ...
    if($('#checklist-'+val).prop("checked")==true){ 
      //alert(val);
        setTimeout(function() {
          var $id = '#reportkq-'+val;
          $($id).html('loading...');
          setTimeout(function() {loadAjax(val)}, 5000);
        }, 5000);
      }
  ...
  function modalShow(){
	...
	function loadAjax(id){
		...
		$.ajax({
			type:"POST", 
      async: false,
			url:'<?php echo Yii::app()->request->baseUrl.'/piadmin/Investeffect/BcOther' ?>', 
			data:{
				id:id,
			},
