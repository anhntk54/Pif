                       ...
                       <!--  <form role="form" action="<?php echo Yii::app()->request->baseUrl?>/Login" method="post"> -->
                           <div class="rows_login" id="rows_login">
                                <div class="form-group">
                                  <label  id="login_email_login" style="color: red;font-size: 85%;font-style: italic">Mật khẩu hoặc Email đã nhập sai!</label>
                                </div>
                                <div class="form-group">
                                    <input class="form-control" placeholder="E-mail" id="email_login" name="email" type="email" >
                                </div>
                                <div class="form-group">
                                     <label   id="err_email_login" style="color: red;font-size: 85%;font-style: italic">Email không được để trống!</label>
                                     <label  id="validate_email_login" style="color: red;font-size: 85%;font-style: italic" >Email không đúng định dang! </label>  
                                </div>
                                <div class="form-group">
                                    <input class="form-control" placeholder="Mật khẩu" name="password" type="password" id="password_login">
                                </div>
                                <div class="form-group">
                                     <label id="err_password_login" style="color: red;font-size: 85%;font-style: italic">Mật Khẩu không được để trống!</label>
                                      <label id="validate_password_login" style="color: red;font-size: 85%;font-style: italic">Mật Khẩu phải dài hơn 6 ký tự</label>
                                </div>
                               <!--  <div class="form-group">
                                   <form id="methodCheckout">
                                    <label>
                                        <input name="remember"  checked  type="radio" value="1"> Quản lý
                                    </label>
                                    <label>
                                        <input name="remember"  type="radio" value="2"> Nhân viên
                                    </label>
                                   </form>
                                </div> -->
                                 ...
                                 <button onclick="login()" class="btn btn-lg btn-success btn-block">Đăng nhập</button> 
                                   <!-- <div class="action_login">
                                      <a onclick="javascript:  $('#rows_login').hide(); $('#rows_forget').show();" title="Quên mật khẩu">Quên mật khẩu?</a><span role="presentation" aria-hidden="true"> · </span>
        ...
        function login(){ 
           ...
           if(password.length<=0) {
            ...
            if(email.length>0 && password.length>0 && IsEmail(email)==true){ 
                    $.ajax({ 
                       type:"POST", 
                       url:'<?php echo Yii::app()->request->baseUrl.'/piadmin/Login/CheckLogin' ?>',
                        data:{
                            email:email,
                            password:password,
